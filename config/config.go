...
func Config() (err error) {
	...
	configuration.MobileConf, err = mobile()
	if err != nil {
		return
	}
	...
	configuration.GoogleConfig, err = googleConfig()

	configuration.FireBaseConfig, err = fireBaseConfig()
...
func database() (databaseConfig DatabaseConfig, err error) {
	...
	// Aerospike
	activateAerospike := strings.TrimSpace(os.Getenv("ACTIVATE_Aerospike"))
	if activateAerospike == Activated {
		dbAerospike, errThis := databaseAerospike()
		if errThis != nil {
			err = errThis
			return
		}
		databaseConfig.AEROSPIKE = dbAerospike.AEROSPIKE
	}
	databaseConfig.AEROSPIKE.Activate = activateAerospike
	...
	// Elastic
	activateElasticSearch := strings.TrimSpace(os.Getenv("ACTIVATE_ELASTIC_SEARCH"))
	if activateElasticSearch == Activated {
		dbElasticSearch, errThis := databaseElasticSearch()
		if errThis != nil {
			err = errThis
			return
		}
		databaseConfig.ELASTICSEARCH = dbElasticSearch.ELASTICSEARCH
	}
	databaseConfig.ELASTICSEARCH.Activate = activateElasticSearch

	// Temporal

	activateTemporal := strings.TrimSpace(os.Getenv("ACTIVATE_TEMPORAL"))
	fmt.Println("Activating Temporal -  %s", activateTemporal)
	if activateTemporal == Activated {
		dbTemporal, errThis := databaseTemporal()
		if errThis != nil {
			err = errThis
			return
		}
		databaseConfig.TEMPORAL = dbTemporal.TEMPORAL
	}
	databaseConfig.TEMPORAL.Activate = activateTemporal
...
// databaseAerospike - all Aerospike variables
func databaseAerospike() (databaseConfig DatabaseConfig, err error) {
	// Load environment variables
	// Load environment variables
	err = Env()
	if err != nil {
	...
	// AEROSPIKE
	poolSize, errThis := strconv.Atoi(strings.TrimSpace(os.Getenv("AEROSPIKE_POOLSIZE")))
	if errThis != nil {
		err = errThis
		return
	}
	connTTL, errThis := strconv.Atoi(strings.TrimSpace(os.Getenv("AEROSPIKE_CONNTTL")))
	if errThis != nil {
		err = errThis
		return
	}

	databaseConfig.AEROSPIKE.Env.Host = strings.TrimSpace(os.Getenv("AEROSPIKE_HOST"))
	port, err := strconv.Atoi(os.Getenv("AEROSPIKE_PORT"))
	if err != nil {
		return
	}
	databaseConfig.AEROSPIKE.Env.Port = port
	databaseConfig.AEROSPIKE.Conn.PoolSize = poolSize
	databaseConfig.AEROSPIKE.Conn.ConnTTL = connTTL

	// Logger
	dbLogLevel := strings.TrimSpace(os.Getenv("DBLOGLEVEL"))
	databaseConfig.AEROSPIKE.Log.LogLevel, err = strconv.Atoi(dbLogLevel)
	if err != nil {
		return
	}
...
// databaseElastic - all ElasticSearch DB variables
func databaseElasticSearch() (databaseConfig DatabaseConfig, err error) {
	// Load environment variables
	err = Env()
	if err != nil {
		return
	...
	// REDIS
	poolSize, errThis := strconv.Atoi(strings.TrimSpace(os.Getenv("ELASTIC_SEARCH_POOL_SIZE")))
	if errThis != nil {
		err = errThis
		return
	}
	connTTL, errThis := strconv.Atoi(strings.TrimSpace(os.Getenv("ELASTIC_SEARCH_CONNTTL")))
	if errThis != nil {
		err = errThis
		return
	}

	databaseConfig.ELASTICSEARCH.Env.Host = strings.TrimSpace(os.Getenv("ELASTIC_SEARCH_HOST"))
	databaseConfig.ELASTICSEARCH.Env.Port = strings.TrimSpace(os.Getenv("ELASTIC_SEARCH_PORT"))
	databaseConfig.ELASTICSEARCH.Env.USERNAME = strings.TrimSpace(os.Getenv("ELASTIC_USERNAME"))
	databaseConfig.ELASTICSEARCH.Env.PASSWORD = strings.TrimSpace(os.Getenv("ELASTIC_PASSWORD"))
	databaseConfig.ELASTICSEARCH.Env.CACERTPATH = strings.TrimSpace(os.Getenv("ELASTIC_CACERTPATH"))
	databaseConfig.ELASTICSEARCH.Conn.PoolSize = poolSize
	databaseConfig.ELASTICSEARCH.Conn.ConnTTL = connTTL

	return
}

// databaseElastic - all Temporal DB variables
func databaseTemporal() (databaseConfig DatabaseConfig, err error) {
	// Load environment variables
	fmt.Println("Activating Temporal Inside")
	...
	// REDIS
	poolSize, errThis := strconv.Atoi(strings.TrimSpace(os.Getenv("TEMPORAL_POOL_SIZE")))
	if errThis != nil {
		err = errThis
		return
	}
	connTTL, errThis := strconv.Atoi(strings.TrimSpace(os.Getenv("TEMPORAL_CONNTTL")))
	if errThis != nil {
		err = errThis
		return
	}

	databaseConfig.TEMPORAL.Env.Host = strings.TrimSpace(os.Getenv("TEMPORAL_HOST"))
	databaseConfig.TEMPORAL.Env.Port = strings.TrimSpace(os.Getenv("TEMPORAL_PORT"))
	databaseConfig.TEMPORAL.Env.USERNAME = strings.TrimSpace(os.Getenv("TEMPORAL_USERNAME"))
	databaseConfig.TEMPORAL.Env.PASSWORD = strings.TrimSpace(os.Getenv("TEMPORAL_PASSWORD"))
	databaseConfig.TEMPORAL.Env.CACERTPATH = strings.TrimSpace(os.Getenv("TEMPORAL_CACERTPATH"))
	databaseConfig.TEMPORAL.Conn.PoolSize = poolSize
	databaseConfig.TEMPORAL.Conn.ConnTTL = connTTL
...
// mobile - config for using external mobile services
func mobile() (mobileConfig MobileConfig, err error) {
	// Load environment variables
	err = Env()
	if err != nil {
		return
	...
	mobileConfig.Activate = strings.TrimSpace(os.Getenv("ACTIVATE_MOBILE_SERVICE"))
	if mobileConfig.Activate == Activated {
		mobileConfig.Provider = strings.TrimSpace(os.Getenv("MOBILE_SERVICE_PROVIDER"))
		mobileConfig.APIToken = strings.TrimSpace(os.Getenv("MOBILE_API_TOKEN"))
		mobileConfig.AddrFrom = strings.TrimSpace(os.Getenv("MOBILE_FROM"))

		mobileConfig.TrackOpens = false
		trackOpens := strings.TrimSpace(os.Getenv("MOBILE_TRACK_OPENS"))
		if trackOpens == "yes" {
			mobileConfig.TrackOpens = true
		}

		mobileConfig.TrackLinks = strings.TrimSpace(os.Getenv("MOBILE_TRACK_LINKS"))
		mobileConfig.DeliveryType = strings.TrimSpace(os.Getenv("MOBILE_DELIVERY_TYPE"))

		mobileConfig.MobileVerificationTemplateID, err = strconv.ParseInt(strings.TrimSpace(os.Getenv("MOBILE_VERIFY_TEMPLATE_ID")), 10, 64)
		if err != nil {
			return
		}
		mobileConfig.MobileVerificationCodeLength, err = strconv.ParseUint(strings.TrimSpace(os.Getenv("MOBILE_VERIFY_CODE_LENGTH")), 10, 64)
		if err != nil {
			return
		}
		mobileConfig.MobileVerificationTag = strings.TrimSpace(os.Getenv("MOBILE_VERIFY_TAG"))
		mobileConfig.HTMLModel = strings.TrimSpace(os.Getenv("MOBILE_HTML_MODEL"))
		mobileConfig.MobileVerifyValidityPeriod, err = strconv.ParseUint(strings.TrimSpace(os.Getenv("MOBILE_VERIFY_VALIDITY_PERIOD")), 10, 64)
		if err != nil {
			return
		}
	}
...
func email() (emailConfig EmailConfig, err error) {
	...
	if emailConfig.Activate == Activated {
		...
		emailConfig.EmailVerificationCodeLength, err = strconv.ParseUint(strings.TrimSpace(os.Getenv("EMAIL_VERIFY_CODE_LENGTH")), 10, 64)
		if err != nil {
			return
		}
		emailConfig.PasswordRecoverCodeLength, err = strconv.ParseUint(strings.TrimSpace(os.Getenv("EMAIL_PASS_RECOVER_CODE_LENGTH")), 10, 64)
		if err != nil {
			return
		}
		...
		emailConfig.EmailVerifyValidityPeriod, err = strconv.ParseUint(strings.TrimSpace(os.Getenv("EMAIL_VERIFY_VALIDITY_PERIOD")), 10, 64)
		if err != nil {
			return
		}
		emailConfig.PassRecoverValidityPeriod, err = strconv.ParseUint(strings.TrimSpace(os.Getenv("EMAIL_PASS_RECOVER_VALIDITY_PERIOD")), 10, 64)
		if err != nil {
			return
		}
	}
...
func security() (securityConfig SecurityConfig, err error) {
	...
	securityConfig.PERSONAL_DATA_ENCRYPTION_KEY = strings.TrimSpace(os.Getenv("PERSONAL_DATA_ENCRYPTION_KEY"))
	securityConfig.GPT_API_KEY = strings.TrimSpace(os.Getenv("GPT_API_KEY"))
...
func server() (serverConfig ServerConfig, err error) {
	...
	serverConfig.ServerPATH = strings.TrimSpace(os.Getenv("APP_BASE_PATH"))

	return
}

func googleConfig() (googleConfig GoogleConfig, err error) {
	err = Env()
	if err != nil {
		return
	}

	googleConfig.Activate = strings.TrimSpace(os.Getenv("ACTIVATE_GOOGLE"))
	if googleConfig.Activate == Activated {
		googleConfig.AccessKey = strings.TrimSpace(os.Getenv("GOOGLE_ACCESS_KEY_ID"))
		googleConfig.PlatformMetricSheet = strings.TrimSpace(os.Getenv("GOOGLE_PLATFORM_MET_SHEET"))
		googleConfig.ContentConversionSheet = strings.TrimSpace(os.Getenv("GOOGLE_CONTENT_C_SHEET"))
		googleConfig.GoogleSheetId = strings.TrimSpace(os.Getenv("GOOGLE_SHEET_ID"))
		googleConfig.Region = strings.TrimSpace(os.Getenv("GOOGLE_REGION"))
		googleConfig.SecreteAccessKey = strings.TrimSpace(os.Getenv("GOOGLE_SECRET_ACCESS_KEY"))
		googleConfig.Path = strings.TrimSpace(os.Getenv("GOOGLE_PATH"))
...
func fireBaseConfig() (fireBaseConfig FireBaseConfig, err error) {
	err = Env()
	if err != nil {
		return
	}

	fireBaseConfig.Activate = strings.TrimSpace(os.Getenv("ACTIVATE_FIREBASE"))
	if fireBaseConfig.Activate == Activated {
		fireBaseConfig.AccessKey = strings.TrimSpace(os.Getenv("FIREBASE_ACCESS_KEY_ID"))
		fireBaseConfig.PlatformMetricSheet = strings.TrimSpace(os.Getenv("GOOGLE_PLATFORM_MET_SHEET"))
		fireBaseConfig.ContentConversionSheet = strings.TrimSpace(os.Getenv("GOOGLE_CONTENT_C_SHEET"))
		fireBaseConfig.GoogleSheetId = strings.TrimSpace(os.Getenv("GOOGLE_SHEET_ID"))
		fireBaseConfig.Region = strings.TrimSpace(os.Getenv("FIREBASE_REGION"))
		fireBaseConfig.SecreteAccessKey = strings.TrimSpace(os.Getenv("FIREBASE_SECRET_ACCESS_KEY"))
		fireBaseConfig.Path = strings.TrimSpace(os.Getenv("FIREBASE_PATH"))

		fireBaseConfig.OneSignalAPPId = strings.TrimSpace(os.Getenv("ONESIGNAL_APPID"))
		fireBaseConfig.OneSignalAPIKey = strings.TrimSpace(os.Getenv("ONESIGNAL_API_KEY"))
...
func aws() (awsConfig AwsConfig, err error) {
	...
	if awsConfig.Activate == Activated {
		...
		awsConfig.InvoiceBucketName = strings.TrimSpace(os.Getenv("S3_INVOICE_BUCKET_NAME"))
		awsConfig.PublicImageBucketName = strings.TrimSpace(os.Getenv("S3_PUBLIC_IMAGE_BUCKET_NAME"))
...
func getParamsHash() (params lib.HashPassConfig, err error) {
	...
	hashPassMemory64, errThis := strconv.ParseUint((strings.TrimSpace(os.Getenv("HASHPASSMEMORY"))), 10, 64)
	if errThis != nil {
		err = errThis
		return
	}
	hashPassIterations64, errThis := strconv.ParseUint((strings.TrimSpace(os.Getenv("HASHPASSITERATIONS"))), 10, 64)
	if errThis != nil {
		err = errThis
		return
	}
	...
	hashPassSaltLength64, errThis := strconv.ParseUint((strings.TrimSpace(os.Getenv("HASHPASSSALTLENGTH"))), 10, 64)
	if errThis != nil {
		err = errThis
		return
	}
	hashPassKeyLength64, errThis := strconv.ParseUint((strings.TrimSpace(os.Getenv("HASHPASSKEYLENGTH"))), 10, 64)
	if errThis != nil {
		err = errThis
		return
	}
