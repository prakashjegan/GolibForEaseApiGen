...
import (
	...
	"github.com/prakashjegan/golangexercise/lib/middleware/middlewarejwt"
)
...
func TestGetJWT(t *testing.T) {
	...
	// valid access token
	accessJWT, _, err := middlewarejwt.GetJWT(customClaims, "access")
	if err != nil {
		t.Errorf("error creating access JWT: %v", err)
	}

	// valid refresh token
	refreshJWT, _, err := middlewarejwt.GetJWT(customClaims, "refresh")
	if err != nil {
...
func TestJWT(t *testing.T) {
	...
	validInFutureAccessJWT, _, err := middlewarejwt.GetJWT(customClaims, "access")
	if err != nil {
	...
	expiredAccessJWT, _, err := middlewarejwt.GetJWT(customClaims, "access")
	if err != nil {
...
func TestRefreshJWT(t *testing.T) {
	...
	validInFutureRefreshJWT, _, err := middlewarejwt.GetJWT(customClaims, "refresh")
	if err != nil {
	...
	expiredRefreshJWT, _, err := middlewarejwt.GetJWT(customClaims, "refresh")
	if err != nil {
	...
	router.POST("/refresh", middlewarejwt.RefreshJWT(), func(c *gin.Context) {
		c.Status(http.StatusOK)
	})
...
func setParamsJWT() {
	...
	middleware.JWTParams.Issuer = "golangexercise"
